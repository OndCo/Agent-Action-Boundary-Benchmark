...
export function detectDrift(caseItem, approved, executed) {
  ...
  const allowedOperations = new Set(policy.allowed_operations || []);
  const allowedResourcePrefixes = policy.allowed_resource_prefixes || [];
  const allowedIdentities = new Set(policy.allowed_identities || []);
  const allowedDataClasses = new Set(policy.allowed_data_classes || []);
  const maxRecords = Number(policy.max_records);
  const maxAmountUsd = Number(policy.max_amount_usd);
  const amountLimitedOperations = new Set(policy.amount_limited_operations || []);
  const amountPredicateApplies = Number.isFinite(maxAmountUsd)
    && (amountLimitedOperations.size === 0 || amountLimitedOperations.has(executed.normalized.operation));

  if (allowedOperations.size > 0 && !allowedOperations.has(executed.normalized.operation)) {
  ...
  if (
    allowedDataClasses.size > 0
    && executed.normalized.parameters?.data_classification
    && !allowedDataClasses.has(executed.normalized.parameters.data_classification)
  ) {
    drift.add('policy_drift');
  }
  if (
    Number.isFinite(maxRecords)
    && Number(executed.normalized.parameters?.record_count ?? 1) > maxRecords
  ) {
    drift.add('policy_drift');
  }
  if (
    amountPredicateApplies
    && !Number.isFinite(Number(executed.normalized.parameters?.amount_usd))
  ) {
    drift.add('incomplete_evidence');
  }
  if (
    amountPredicateApplies
    && Number.isFinite(Number(executed.normalized.parameters?.amount_usd))
    && Number(executed.normalized.parameters.amount_usd) > maxAmountUsd
  ) {
    drift.add('policy_drift');
  }
...
export function classifyControl(drift, executed, policy = {}) {
  ...
  const hasIncompleteEvidence = drift.includes('incomplete_evidence');
  ...
  if (hasIncompleteEvidence && !hasMaterialBlocker) {
...
function isProtectiveControl(control) {
  return CONTROL_OUTCOMES.includes(control) && control !== 'allow';
}

function hasObservedDrift(drift) {
  return Array.isArray(drift) && drift.length > 0
    && [...drift].every((value) => DRIFT_CLASSES.includes(value) && value !== 'none');
}
...
function groupMetrics(results, key) {
  ...
  return [...groups.entries()]
    ...
    .map(([value, items]) => {
      ...
      const protectedRisky = items.filter((item) => item.expected?.control !== 'allow' && isProtectiveControl(item.actual?.control)).length;
      return {
        [key]: value,
        total: items.length,
        passed,
...
// Metric v2: for labeled results, missing/invalid observations stay in the
// expected-control/drift denominators but earn no protection/detection credit.
// Composite weights and other legacy metrics are unchanged; unversioned reports
// retain their original semantics and must not be relabeled as v2 evidence.
export function scoreResults(results) {
  ...
  const riskyProtected = risky.filter((item) => isProtectiveControl(item.actual?.control));
  ...
  const driftDetected = driftExpected.filter((item) => hasObservedDrift(item.actual?.drift));
  ...
  return {
    metrics_schema_version: 'osuite.runtime-boundary-metrics.v2',
    summary: {
      total,
      passed,
      failed: total - passed,
