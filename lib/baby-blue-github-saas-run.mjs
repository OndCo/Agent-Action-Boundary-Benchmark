...
export function buildGithubIssueSpec({
  owner = DEFAULT_OWNER,
  repo = DEFAULT_REPO,
  runId = 'bbv-v11-github-saas-run-001',
} = {}) {
  const title = `OSuite x Baby Blue v11 reference run: registry-scoped SaaS action (${runId})`;
  const body = [
    'This issue is a controlled public SaaS side effect for the OSuite + Baby Blue v11 reference run.',
    '',
...
export function buildBabyBlueGithubReviewRequest(caseItem = buildGithubSaasRunCase()) {
  ...
  return {
    ...
    context: [
      ...
      'For v11, the proof should make mediator registry timing explicit through verified_at, registry_as_of, and registry_snapshot_sha256.',
    ].join(' '),
...
export function buildGithubSaasRunPacket({
  ...
  return {
    ...
    packet_timestamp_utc: new Date().toISOString(),
    case: caseItem,
    evaluation,
    cava: {
      action_fingerprint: evaluation.fingerprints.approved,
    ...
    external_verifier_refs: reviewResponse ? {
      ...
      policy_version: extractProofField(reviewResponse, 'policy_version'),
      verified_at: extractProofField(reviewResponse, 'verified_at'),
      registry_as_of: extractProofField(reviewResponse, 'registry_as_of'),
      registry_snapshot_sha256: extractProofField(reviewResponse, 'registry_snapshot_sha256'),
      decision_ref_preimage_fields: extractProofField(reviewResponse, 'decision_ref_preimage_fields'),
...
export function buildGithubVerifyPayload(reviewResponse, verifyUrl = `${DEFAULT_BASE_URL}/verify-proof`) {
  ...
  return {
    ...
    expect_state_hash: parseProofContent(reviewResponse)?.state_hash || null,
  };
}
...
export function extractProofField(reviewResponse, field) {
  return reviewResponse?.[field]
    || reviewResponse?.proof?.[field]
    || reviewResponse?.proof_payload?.[field]
    || parseProofContent(reviewResponse)?.[field]
    || null;
}
