...
// Metric v2 conditions safe fast coverage on expected allows and false fast
// allows on expected non-allows (including review/dual approval). Counts expose
// both fractions. Rates retain four-decimal rounding and the legacy zero for an
// empty denominator; that zero is unavailable evidence, not measured success.
// This versions metrics only, not Action Pass objects or frozen v0.1 reports.
export function scoreActionPassLaneResults(results) {
  ...
  const expectedDisallowed = results.filter((result) => result.expected.control !== 'allow');
  const fastAllowed = results.filter((result) => result.actual.lane === 'fast' && result.actual.control === 'allow');
  const safeFastAllows = fastAllowed.filter((result) => result.expected.control === 'allow');
  const falseFastAllows = fastAllowed.filter((result) => result.expected.control !== 'allow');
  ...
  return {
    metrics_schema_version: 'osuite.action-pass-lane-metrics.v2',
    total_cases: total,
    passed,
    failed: total - passed,
    ...
    expected_allowed_cases: expectedAllowed.length,
    expected_disallowed_cases: expectedDisallowed.length,
    safety_preserving_fast_allows: safeFastAllows.length,
    false_fast_allows: falseFastAllows.length,
    safety_preserving_fast_path_coverage: percent(safeFastAllows.length, expectedAllowed.length),
    false_fast_allow_rate: percent(falseFastAllows.length, expectedDisallowed.length),
